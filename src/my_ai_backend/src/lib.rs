...
pub async fn commit_batch(
    ...
    for chunk_id in chunk_ids.clone() {
        ...
        match content {
            ContentType::Weight => {
                STATE
                    .with(|s| s.borrow_mut().stable_data.get(&model_name))
                    .unwrap()
                    .0
                    .weight
                    .unwrap()
        ...
        ic_cdk::println!("extended llm content. chunk id: {:?}", chunk_id);
    }
...
#[query]
fn get_llm_ditaile(key: String) -> Option<(LLM, Option<usize>, Option<usize>)> {
    let llm_result = STATE.with(|s| s.borrow().stable_data.get(&key));
    match llm_result {
        Some(llm) => {
            let weight_len = match &llm.0.weight {
                Some(weight) => Some(weight.len()),
                None => None,
            };
            let tokenizer_len = match &llm.0.tokenizer {
                Some(tokenizer) => Some(tokenizer.len()),
                None => None,
            };
            Some((llm.0.clone(), weight_len, tokenizer_len))
        }
        None => None,
    }
}

fn remove_tpmp_content(count_pat: CountPat, model_name: String, content_type: String) -> bool {
    match count_pat {
        CountPat::Pat0 => STATE.with(|s| {
            s.borrow_mut()
                .tmp_data0
...
pub fn remove_tmp_data(model_name: String, content_type: String) -> (String, Vec<bool>) {
    ...
    for i in 0..MAX_COUNT_PAT {
        ...
        result.push(remove_tpmp_content(
            count_pat.clone(),
            model_name.clone(),
            content_type.clone(),
        ));
