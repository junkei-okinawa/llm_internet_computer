...
pub struct ModelContent {
    pub key: String,
    pub content: Vec<u8>,
    pub total_chunks_length: Nat,
    pub current_chunks_length: Nat,
    pub total_content_length: u64,
    pub current_content_length: u64,
    pub modified: Timestamp,
}
...
pub struct State {
    ...
    pub model_store: HashMap<String, ModelContent>,
...
impl State {
    pub fn get_asset_encodings_chunks(&self, arg: GetArg) -> Result<Vec<RcBytes>, String> {
        let content_encoding = &arg.accept_encodings[0];
        let get_asset_result = self.get_asset(&arg.key);
        let asset = match get_asset_result {
            Ok(a) => a,
            Err(e) => return Err(e),
        };

        let enc_asset_result = asset
            .encodings
            .get(content_encoding)
            .ok_or_else(|| format!("no {} encoding", content_encoding));
        let enc = match enc_asset_result {
            Ok(e) => e,
            Err(e) => return Err(e),
        };

        Ok(enc.content_chunks.clone())
    }
    // customizing ======================
    pub fn get_model_content(&mut self, key: &String) -> Result<Vec<u8>, String> {
        match self.model_store.get_mut(key) {
            Some(model_content) => {
                ic_cdk::println!("get_model_content. model_content: {:?}", model_content);
                Ok(model_content.content.clone())
            }
            None => Err("model content not found".to_string()),
        }
    }
    // customizing ======================

    // customizing ======================
    pub fn get_content_asset_encoding(&mut self, arg: GetArg) -> Result<Vec<u8>, String> {
        match self.assets.get_mut(&arg.key) {
            Some(asset) => {
                ic_cdk::println!("get asset.");
                match asset.encodings.get(&arg.accept_encodings[0]) {
                    Some(encoding) => {
                        ic_cdk::println!("get encoding.");
                        Ok(encoding
                            .content_chunks
                            .iter()
                    ...
                    None => Err("encoding not found".to_string()),
                }
            }
    ...
    pub fn set_model_content(
        &mut self,
        arg: SetModelContentArguments,
    ) -> Result<SetModelContentResponse, String> {
        ic_cdk::println!("call!! set_model_content.==========");
        if !self.model_store.contains_key(&arg.key) {
            return Err("model content not found".to_string());
        };

        let model_content = self.model_store.get_mut(&arg.key).unwrap();
        ic_cdk::println!("get model_content.");

        // if model_content.current_chunks_length.is_some()
        //     && model_content.total_chunks_length
        //         == model_content.clone().current_chunks_length.unwrap()
        // {
        //     return Err("model content is already complete".to_string());
        // };
        // ic_cdk::println!("section 1");

        // let total_chunks_length = model_content
        //     .total_chunks_length
        //     .clone()
        //     .0
        //     .to_usize()
        //     .unwrap();
        // ic_cdk::println!("section 2");

        // // get content chunks
        // let start = if model_content.current_chunks_length.is_none() {
        //     ic_cdk::println!(
        //         "set_model_content vec. total_chunks_length: {}",
        //         total_chunks_length
        //     );
        //     0usize
        // } else {
        //     model_content
        //         .clone()
        //         .current_chunks_length
        //         .unwrap()
        //         .0
        //         .to_usize()
        //         .unwrap()
        // };
        // ic_cdk::println!("section 3");

        // let end = if total_chunks_length > start + 50 {
        //     start + 50
        // } else {
        //     total_chunks_length
        // };
        // ic_cdk::println!("section 4");
        ...
        for _ in 0..arg.set_content_count {
            let target_index = model_content.current_content_length;
            if target_index >= model_content.total_content_length {
                ic_cdk::println!("set_model_content. break");
                break;
            };
            ic_cdk::println!("set_model_content. target_index: {}", target_index);
            let bytes_content = content_chunks
                .get(model_content.current_chunks_length.0.to_usize().unwrap())
                .unwrap();
            model_content.current_content_length += bytes_content.len() as u64;
            model_content.current_chunks_length += Nat::from(1u8);
            model_content
                .content
                .extend_from_slice(bytes_content.as_ref());
        }
        ic_cdk::println!(
            "model_content.content.len(): {:?}",
            model_content.content.len()
        );

        let res = SetModelContentResponse {
            total_chunks_length: model_content
                .total_chunks_length
                .clone()
                .0
                .to_u32()
                .unwrap(),
            current_chunks_length: model_content
                .current_chunks_length
                .clone()
                .0
                .to_u32()
        ...
        model_content.modified = Int::from(arg.now);
        Ok(res)
    }
    // customizing ======================
    ...
    pub fn create_asset(&mut self, arg: CreateAssetArguments) -> Result<(), String> {
        ...
        // customizing ======================
        let model_content = ModelContent {
            key: arg.key.clone(),
            content: vec![], // empty
            total_chunks_length: Nat::from(0u8),
            current_chunks_length: Nat::from(0u8),
            total_content_length: 0u64,
            current_content_length: 0u64,
            modified: Int::from(ic_cdk::api::time()),
        };
        ic_cdk::println!("insert model_content: {:?}", model_content);
        self.model_store.insert(arg.key.clone(), model_content);
        // customizing ======================
    ...
    pub fn set_asset_content(
        ...
        ic_cdk::println!("set_asset_content. arg: {:?}", arg);
        ...
        let asset = self
            ...
            .get_mut(&arg.key)
            .ok_or_else(|| "asset not found".to_string())?;
        ic_cdk::println!("get asset: {:?}", asset);

        let now = Int::from(now);

        let mut total_length = 0;
        let mut content_chunks = Vec::with_capacity(arg.chunk_ids.len());
        ...
        ic_cdk::println!("sha256: {:?}", sha256);

        // customizing ======================
        let model_content = self.model_store.get_mut(&arg.key).unwrap();

        model_content.total_chunks_length = Nat::from(arg.chunk_ids.len() as u64);
        model_content.total_content_length = total_length as u64;
        ic_cdk::println!("total_length: {:?}", total_length);
        // model_content.content = Vec::with_capacity(total_length);

        ic_cdk::println!("set model_content.");
        // customizing ======================
    ...
    pub fn delete_asset(&mut self, arg: DeleteAssetArguments) {
        ...
        // customizing ======================
        ic_cdk::println!("delete model_store: {}", arg.key);
        self.model_store.remove(&arg.key);
        // customizing ======================
    }
    ...
    pub fn store(&mut self, arg: StoreArg, time: u64) -> Result<(), String> {
        ...
        // customizing ======================
        ic_cdk::println!("store model_store: {}", arg.key);
        let model_content = ModelContent {
            key: arg.key.clone(),
            content: arg.content.as_ref().to_vec(),
            total_chunks_length: Nat::from(1u8),
            current_chunks_length: Nat::from(1u8),
            total_content_length: arg.content.len() as u64,
            current_content_length: arg.content.len() as u64,
